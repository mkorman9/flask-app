...
class TodoItemsPage(object):
    def __init__(self, data: List[TodoItem], page_size: int, next_page_token: Optional[uuid.UUID] = None):
        self.data = data
    ...
    try:
        with pool.connection() as conn:
            ...
            with conn.execute(*query) as result:
                data = [TodoItem(item_id=record[0], content=record[1]) for record in result.fetchall()]
                next_page_token = data[len(data) - 1].item_id if len(data) > 0 else None
